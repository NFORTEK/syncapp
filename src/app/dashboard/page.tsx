...
export default function Dashboard() {
  ...
  const fetchPlanData = async () => {
    try {
      const response = await fetch('https://api.ecosentry.cloud/v1/getPlan', {
        method: 'GET',
  ...
  const fetchUserConnections = async () => {
    try {
      const response = await fetch('https://api.ecosentry.cloud/v1/user-connections', {
        method: 'GET',
        credentials: 'include',
      });
      const data = await response.json();
  ...
  const saveAdminCredentials = async () => {
    try {
      const response = await fetch('https://api.ecosentry.cloud/v1/save-credentials', {
        method: 'POST',
        credentials: 'include', // Para enviar os cookies de autenticação
        headers: {
